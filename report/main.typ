...
reckoning to guide museum visits @theodorou2024, and the broader outdoor-navigation app
landscape surveyed by El-Taher et al., who catalogue dozens of point solutions but note
none integrate trip-level planning @eltaher2023. The second category is general-purpose AI
...
AR landmark recognition were brainstormed but deferred — the former to a future-scaling
claim rather than a v1 feature, the latter folded into the Wearable Extension's scope.
...
The name CapeScape pairs the destination (Cape Town) with "-scape," signalling a
constructed view or journey rather than a single place. The domain model keeps "Destination"
— not "Cape Town" — as the first-class entity name throughout (`docs/adr/0001`), so the
name reads as scope for this prototype rather than a hardcoded limit: adding a second
Destination is a content addition to the same model, not a rename.
